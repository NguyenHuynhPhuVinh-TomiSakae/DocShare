...
const steps = [
  ...
  {
    icon: <Share1Icon className="h-8 w-8" />,
    title: "Chia sẻ",
  ...
  {
    icon: <UpdateIcon className="h-8 w-8" />,
    title: "Cập nhật",
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="absolute -top-40 -left-40 w-80 h-80 bg-blue-500/10 rounded-full blur-[100px]"></div>
        <div className="absolute -bottom-40 -right-40 w-80 h-80 bg-purple-500/10 rounded-full blur-[100px]"></div>
      </div>
        ...
        >
          {steps.map((step, index) => (
            <motion.div
              ...
              className="bg-zinc-900/80 backdrop-blur-sm border border-zinc-800 rounded-xl p-8 hover:border-zinc-700 hover:shadow-lg hover:shadow-blue-900/5 transition-all duration-300 h-full flex flex-col"
            >
              <div className={`w-20 h-20 rounded-2xl bg-gradient-to-r ${step.color} flex items-center justify-center mb-8 mx-auto shadow-lg`}>
                {step.icon}
              </div>
              <h3 className="text-2xl font-semibold mb-4 text-center">{step.title}</h3>
              <p className="text-zinc-400 text-center text-lg">{step.description}</p>
            </motion.div>
          ))}
        </motion.div>
      </div>
